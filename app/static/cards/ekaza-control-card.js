(function () {
  ...
  const CARD_TYPE = 'ekaza-control-card';
  const VERSION   = '3.5.1';
  ...
  customElements.define('ekaza-control-card-editor', EkazaCameraCardEditor);
  ...
  class EkazaCameraCard extends HTMLElement {
    ...
    static getConfigElement() {
      return document.createElement('ekaza-control-card-editor');
    }
    ...
    _wireEvents() {
      ...
      on('scc-snap', () => { const url = this._snapshotUrl(); if (url) window.open(url, '_blank'); });
      ...
      ctrl.addEventListener('change', ev => {
        ...
        if (el.dataset.detmode) {
          this._setDetectionMode(el.value);
          return;
        }
  ...
  console.info(
    `%c EKAZA-CONTROL-CARD %c v${VERSION} `,
