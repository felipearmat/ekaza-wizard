...
async def _find_localtuya_device(hass: HomeAssistant, slug: str) -> tuple[str | None, str | None]:
...
async def _remove_from_localtuya(hass: HomeAssistant, device_id: str) -> tuple[bool, str]:
    removed: list[str] = []
    for entry in hass.config_entries.async_entries("localtuya"):
...
async def remove(
    hass: HomeAssistant, slug: str, device_id: str | None = None,
) -> AsyncGenerator[str, None]:
    """Remove a provisioned camera. Yields SSE events for each step.

    Total: 8 steps
      1. bridge         — stop motion bridge thread
      ...
      7. motion_bridge_boolean — delete input_boolean helper
    ...
    yield _sse("step", {
        ...
        "ok": reloaded,
        "detail": "Integração Frigate recarregada — entidades de câmera removidas" if reloaded
    ...
    try:
        if scripts_path.exists():
            scripts_path.unlink()
            yield _sse("step", {"step": "scripts", "ok": True, "detail": "Scripts PTZ removidos"})
        else:
            yield _sse("step", {"step": "scripts", "ok": True, "detail": "Scripts PTZ não encontrados (ok)"})
    except Exception as exc:
    ...
    try:
        ...
        yield _sse("step", {"step": "reload_scripts", "ok": True, "detail": "Scripts HA recarregados"})
    except Exception as exc:
    ...
    if device_id:
        ...
        ok, msg = True, "Dispositivo não encontrado no LocalTuya (já removido ou não configurado)"
